...
- `switch`, `fallback`, `rotate`, `random`: replaced parallel list parameters
  ...
  methods (`on_select`, `on_leave`, `single`, `replay_metadata`). `on_select`
  receives the incoming source and returns a processed source (e.g. with a
  fade-in or intro jingle); `on_leave` is called with the source being left.
  `replay_metadata` and `single` are deprecated with backward-compatible

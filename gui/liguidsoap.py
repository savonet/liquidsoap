...
bg = request.equeue (id="background")
music = request.equeue (id="music")
sfx = request.equeue (id="sfx")
micro = input.alsa (id="microphone")
mixer = mix(id="mixer",[micro,music,bg,sfx])

output.alsa(id="speaker",mixer)
output.icecast(
  id="broadcast",
  host="%s",port=%d,mount="%s",bitrate=64,start=false,mixer)
output.ogg(id="local backup",start=false,"%s",mixer)

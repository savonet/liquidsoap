...
The `transitions` list parameter and `transition_length`/`override` parameters have been removed. Per-source selection behavior is now controlled through composition methods on each source:

- `on_select`: called when the source is selected. Receives `{starting, ending}` and returns the source to play. `ending` is `null` when the switch happened at a track boundary; non-null when the old source was preempted mid-track. By default, file sources return `starting` unchanged; live sources fade out `ending` when non-null.
...
```liquidsoap
def my_on_select({ending, starting}) =

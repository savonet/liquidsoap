...
- **Instantiation**: when script is executed, sources get created. Remaining unknown [stream types](./stream_content.md) are forced according to `frame.*.channels` settings, [clocks](./clocks.md) are assigned (but unknown clocks may remain) and some sources are checked to be [infallible](./sources.md). Each of these steps may raise an error.

...
- **Instantiation**: when script is executed, sources get created. Remaining unknown [stream types](stream_content.html) are forced according to `frame.*.channels` settings, [clocks](clocks.html) are assigned (but unknown clocks may remain) and some sources are checked to be [infallible](sources.html). Each of these steps may raise an error.

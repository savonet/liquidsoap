...
fun (
  {
    source,                 # source  — the live source
    fallback,               # source? — fallback source, or null
    mime_type,              # string? — MIME type reported by the source
    on_listener_connect,    # listener connect callback
    on_listener_disconnect  # listener disconnect callback
  }
) ->
  {clock, shutdown}  # must return this; output.harbor returns it directly
```

`prepare` is a two-stage closure. The first call happens at mount creation time (when the first source connects and no output exists yet) and receives mount-level configuration. The returned function is called immediately after with source-specific data. The final return value must include at least `clock` (used to synchronise the server clock) and `shutdown`; `output.harbor` returns this directly.

### Example: Adding a video stream when the sender sends audio only

The first stage receives `streams`, which describes what the source is sending. This lets you inspect the incoming stream and build the output accordingly:
...
When you only want to change the container format without touching the encoded data, provide a `prepare` function that calls `copy_encoder` with the target format name:

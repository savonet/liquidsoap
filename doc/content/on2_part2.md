...
with liquidsoap using `requests.dynamic`.

`requests.dynamic` takes a function of type `()->[request('a)]`,
...
We are going to use `requests.dynamic` to merge both the `songs` and 
...
```liquidsoap
s = requests.dynamic(id="s",get_request)
```
...
* [radio.liq](radio.liq-requests.dynamic)
* [library.liq](library.liq-requests.dynamic)
...
```
Finally, we apply `map_metadata` to the source, just after the `requests.dynamic`
...
passed to `requests.dynamic` to something of the form:
```
...
```
And, we add the following line in `radio.liq` after the `requests.dynamic` line:
```liquidsoap
...
have added when creating the `requests.dynamic` source.
...
parameters of the `requests.dynamic` source.

  ...
  let ic = open_in_bin file in
    try
      ...
      if buff <> "RIFFWAVEfmt " then raise (Not_a_wav_file 
        "[EE] Bad header : string \"RIFF\", \"WAVE\" or \"fmt \" not found") ;
       ...
       if debug && chan_num <> 2 then  Printf.printf
	   "[WW] Bad channel number: will be converted to stereo\n";
       let samp_hz = read_int ic in
       if debug && samp_hz <> 44100 then	 Printf.printf
	   "[WW] Bad sample frequence: will be converted to 44100 Hz\n";
       let byt_per_sec = read_int ic in
       let byt_per_samp= read_short ic in
       if debug && byt_per_samp <> 4 then Printf.printf
	   "[WW] Bad bytes per sample number: will be converted to 4\n";
       let bit_per_samp= read_short ic in
       if debug && bit_per_samp <> 16 then Printf.printf
	   "[WW] Bad bits per sample: will be converted to 16\n";
       flush stdout;

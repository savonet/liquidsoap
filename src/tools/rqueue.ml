...
let top q =
  ...
  match q.length with
    | 0 ->
        Mutex.unlock q.lock ; raise Not_found
    | _ ->
        let ans = q.top.content in
        Mutex.unlock q.lock ; ans
...
let to_list q =
  Mutex.lock q.lock ;
  if q.length = 0 then (Mutex.unlock q.lock ; [])
  else (
    let rec aux ans c =
...
let fold f x q =
  Mutex.lock q.lock ;
  if q.length = 0 then (Mutex.unlock q.lock ; x)
  else (
    let rec fold c x =

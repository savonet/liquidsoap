...
(** [add command f] adds a new command [command] in a given namespace. When
  * the command is called, the function [f] is executed with the arguments of
  * the command as parameter. The return value of [f] is then displayed. *)
val add : ns:namespace -> ?usage:string -> string -> (string -> string) -> unit
...
(** {2 Multi-socket event engine} *)

(** A task for the scheduler. *)
type task =

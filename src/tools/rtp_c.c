...
CAMLprim value liquidsoap_new_session(value _mode, value _ip, value _port) {
  CAMLparam3(_mode, _ip, _port);
  ...
  int zero = 0;               // For setsockopt
  struct ip_mreqn mreq;       // For setsockopt
  int local_listeners = 1;    // Send the packets to the local sockets only
...
#define BYTES_PER_BUFFER (4*1024)
...
#endif
  if(err<=0) String_val(_buffer)[0]='\0';

  if(hm) {
    fprintf(stderr," *** There's garbage !\n");
    // rtp_session_recv_with_ts(s,buffer,BYTES_PER_BUFFER,*ts,&hm);
  }

  *ts += TICKS_PER_BUFFER ;

  CAMLreturn(Val_bool(hm));
}
...
liquidsoap_send_metadata(value _session, value _string)
{
  ...
  int err =
#endif
    rtp_session_send_with_ts(s,String_val(_string),size,ts);
...
liquidsoap_recv_metadata(value _session)
{
  ...
  while(hm) {
    ...
    err = rtp_session_recv_with_ts(s,block+offset,allocated-offset,ts,&hm);
    if (err>0) offset+=err;
#ifdef DEBUG

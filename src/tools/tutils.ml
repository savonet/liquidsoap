  ...
  mutexify lock (
    fun () ->
        ...
        Thread.create
          (fun x ->
             try
               ...
               log 3 (Log.f "thread %S exited (%d remaining)"
                        s (Set.cardinal !all)) ;
               Mutex.unlock lock
	     with e ->
	       Mutex.lock lock ; 
	       begin match e with
	       | Failure e ->
                   log 1 (Log.f "thread %S failed: %s" s e)
	       | e ->
                   log 1 (Log.f "thread %S aborts with exception %s !" 
      ...
      in
        ...
        log 3 (Log.f "Created thread %S (%d total)" s (Set.cardinal !all)) ;
	id
  ) ()
  ...
  let rec scheduler () =
    ...
    begin try
      ...
      let t = Queue.pop active in
        begin match !t with
          ...
          | Yield k ->
              ...
              begin try t := k () with
                | e ->
                    uncaught := Some e ;
                    log 1 (Log.f "Task aborts with exception %s" 
        ...
        Mutex.unlock global_sleep
    with
      | Queue.Empty -> Mutex.unlock lock ; log 4 "Scheduler sleeps"
    end ;
    scheduler ()

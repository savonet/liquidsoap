...
let duration process = 
  ...
  let w = Iff.in_chan_read_header pull in
...
let create_stream process input =
  ...
  let ret = Iff_decoder.D_stream.create input in
...
let external_input_oblivious process filename prebuf = 
  ...
  let decoder = Iff_decoder.D.create input in
  let fill frame = 
     if not !process_done then
       begin try

...
module Buffered (Generator : Generator.S) = struct
  let make_file_decoder ~filename ~close ~kind ~remaining decoder gen =
    ...
    let prebuf =
      (* Amount of data to decode in advance, in ticks.
         This value was historically very high (0.5s) and
         was too much for video. Setting it to 5 frames for
         now. -- @toots *)
      5 * Lazy.force Frame.size
    in

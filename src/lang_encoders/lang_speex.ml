  ...
  let speex =
    List.fold_left
      (fun f -> function "stereo", {term= Bool b; _} ->
            {f with Speex_format.stereo= b} | "mono", {term= Bool b; _} ->
        ...
        | "mode", {term= Var s; _} when String.lowercase_ascii s = "narrowband"
        ...
        | "bytes_per_page", {term= Int i; _} ->
            {f with Speex_format.fill= Some i} | "dtx", {term= Bool b; _} ->
            {f with Speex_format.dtx= b} | "vad", {term= Bool b; _} ->
        ...
        | "", {term= Var s; _} when String.lowercase_ascii s = "stereo" ->
            {f with Speex_format.stereo= true} | _, t ->

  ...
  sig
    ...
    module Duppy :
      sig
        ...
        module Monad :
          sig
            module Io : Duppy.Monad.Monad_io_t with type socket = socket
              and module Io = Io
      ...
      protocol: string ->
        data: string ->
          headers: ((string * string) list) ->
            socket: socket -> string -> (reply, reply) Duppy.Monad.t
      ...
      kind: Frame.content_kind ->
        object
          ...
          method virtual relay :
            string ->
              (string * string) list ->
                ?read: (socket -> int -> (Bytes.t * int)) -> socket -> unit
...
module Make (T : Transport_t) : T with type socket = T.socket =
  struct
    ...
    let string_of_protocol : protocol -> string =
      function
      | `Http_10 -> "HTTP/1.0"
      | `Http_11 -> "HTTP/1.1"
      | `Ice_10 -> "ICE/1.0"
          ...
          (fun s -> (* Authentication can be blocking. *)
             Duppy.Monad.Io.exec ~priority: Tutils.Maybe_blocking h
               (let (user, auth_f) = s#login in
                let user =
                  if requested_user = "" then user else requested_user
                in
                  if auth_f user password
    ...
    let auth_check ~auth_f user pass = (* OK *)
    ...
    let http_auth_check ?args ~login headers = (* 401 error model *)
      let http_reply s =
        reply
          (http_error_page 401
             "Unauthorized\r\n\
          ...
          let (user, pass) =
            ...
            | Not_found ->
                (match args with
                 ...
                 | _ -> raise Not_found)
          in auth_check ~auth_f user pass
        with
        | Not_authenticated ->
                  ...
                  try
                    ...
                    in
                      (log#f 4 "%s request on %s." sproto uri;
                       ...
                       in
                         (log#f 4
                            "Adding source on mountpoint %S with type %S."
                            uri stype;
                          log#f 5 "Relaying %s."
    ...
    let handle_websocket_request ~port h mount headers =
      let json_string_of =
        function | `String s -> s | _ -> raise Not_found in
                ...
                (log#f 5 "Hello packet: %s\n%!" s;
                 (match extract_packet s with
                  | ("hello", data) ->
                      ...
                      in Duppy.Monad.return (mime, mount, user, password)
                  | _ -> error ()))
            | _ -> error ()
          with | _ -> error ()
      in
           ...
           (Hashtbl.iter (fun h v -> log#f 4 "HTTP Arg: %s, value: %s." h v)
            ...
            let f (verb, reg_uri) handler =
              ...
              in
                ...
                then
                  (log#f 4 "Found handler '%s %s' on port %d." smethod
                     reg_uri port;
                 ...
                 (fun (hmethod, huri, hprotocol) ->
                    let hprotocol =
                      try
                        (if
                           (hmethod = `Get) &&
                             ((assoc_uppercase "UPGRADE" headers) <>
                                "websocket")
                         then raise Exit
                         else ();
                         if
                      ...
                      in
                        Duppy.Monad.bind __pa_duppy_0
                          (fun (auth, huri, smethod) ->
                             handle_source_request ~port ~auth ~smethod
                               ...
                               in
                                 Duppy.Monad.bind __pa_duppy_0
                                   (fun s ->
                                      ...
                                      in
                                        handle_source_request ~port
                                          ~auth: true ~smethod: `Shout
                                          hprotocol h huri headers))
                      | _ ->
    ...
    let open_port ~icy port =
      (log#f 4 "Opening port %d with icy = %b" port icy;
       ...
       let process_client sock =
         try
           ...
           in
             (log#f 4 "New client on port %i: %s" port ip;
              let unix_socket = T.file_descr_of_socket socket
              in
                 ...
                 let reply r =
                   ...
                   in
                     Duppy.Io.write ~timeout: conf_timeout#get
                       ...
                       ~string: (Bytes.of_string s) ~exec Tutils.scheduler
                       socket
                 in
         ...
         with
         | e ->
             log#f 2 "Failed to accept new client: %s" (Printexc.to_string e) in
       let rec incoming ~port ~icy events out_s e =
         ...
         then
           ...
           (let get_sock = function | `Read sock -> sock | _ -> assert false
            in
       ...
       let bind_addrs =
         List.fold_left
           ...
           [] conf_harbor_bind_addrs#get in
       let (in_s, out_s) = Unix.pipe () in
       let events = (`Read in_s) :: (List.map (open_socket port) bind_addrs)
       in

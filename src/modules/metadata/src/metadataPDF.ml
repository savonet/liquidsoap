            ...
            if i + 1 >= len then (
              ...
              match content.[i + 1] with
                ...
                | d when is_digit d ->
                    ...
                    let oct_str =
                      String.sub content (i + 1) (end_oct - (i + 1))
                    in
                    (* Take up to 3 digits *)
                    let oct_len =
                      let rec count k =
                        if
                          k < String.length oct_str
                          && is_octal_digit oct_str.[k]
                        then count (k + 1)
                        else k
                      in
                      count 0
                    ...
                    if oct_len > 0 then (
                      let code =
                        int_of_string ("0o" ^ String.sub oct_str 0 oct_len)
                      in
      ...
      if val_start >= len then ()
        ...
        match content.[val_start] with
          | '(' ->
              ...
              let raw_val =
                String.sub content (val_start + 1) (val_end - (val_start + 1))
              in
...
let recode s =
  if String.starts_with ~prefix:"\xfe\xff" s then
    MetadataCharEncoding.Naive.convert ~source:`UTF_16 s
...
let find f s =
  ...
  let find_newline () =
    while not (is_newline @@ (R.read f 1).[0]) do
      ()
    done
  in
...
let parse f : metadata =
  ...
  parse_metadata obj |> List.map (fun (k, v) -> (k, recode v))

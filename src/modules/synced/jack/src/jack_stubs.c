...
static struct custom_operations client_ops = {
    "liquidsoap_jack_client",   client_finalize,
...
static struct custom_operations port_ops = {
    "liquidsoap_jack_port",     custom_finalize_default,
    custom_compare_default,     custom_hash_default,
    custom_serialize_default,   custom_deserialize_default,
    custom_compare_ext_default, custom_fixed_length_default};
...
CAMLprim value caml_jack_port_get_buffer(value _port, value _frame_count) {
  ...
  float *audio_buffer =
      (float *)jack_port_get_buffer(Port_val(_port), frame_count);
  intnat dimensions[1];
  dimensions[0] = (intnat)frame_count;
  _audio_bigarray =
      caml_ba_alloc(CAML_BA_FLOAT32 | CAML_BA_C_LAYOUT | CAML_BA_EXTERNAL, 1,
                    audio_buffer, dimensions);
...
static struct custom_operations rb_ops = {
    "liquidsoap_jack_ringbuffer", rb_finalize,
    custom_compare_default,       custom_hash_default,
    custom_serialize_default,     custom_deserialize_default,
    custom_compare_ext_default,   custom_fixed_length_default};
...
CAMLprim value caml_jack_ringbuffer_create(value _size) {
  ...
  jack_ringbuffer_t *ringbuffer =
      jack_ringbuffer_create((size_t)Int_val(_size));
  if (!ringbuffer)
    caml_failwith("jack_ringbuffer_create: failed");
  _ringbuffer_block =
      caml_alloc_custom(&rb_ops, sizeof(jack_ringbuffer_t *), 0, 1);
...
CAMLprim value caml_jack_ringbuffer_mlock(value _ringbuffer) {
  ...
  if (result != 0)
    caml_failwith("jack_ringbuffer_mlock: failed");
  CAMLreturn(Val_unit);
}

CAMLprim value caml_jack_ringbuffer_read(value _ringbuffer, value _buffer,
                                         value _offset, value _count) {
  size_t bytes_read = jack_ringbuffer_read(
      Rb_val(_ringbuffer), (char *)Bytes_val(_buffer) + Int_val(_offset),
...
CAMLprim value caml_jack_ringbuffer_write(value _ringbuffer, value _buffer,
                                          value _offset, value _count) {
  size_t bytes_written = jack_ringbuffer_write(
      Rb_val(_ringbuffer), (const char *)Bytes_val(_buffer) + Int_val(_offset),
      (size_t)Int_val(_count));
 ...
 */
CAMLprim value caml_jack_ringbuffer_read_ba(value _ringbuffer, value _bigarray,
                                            value _offset, value _count) {
  ...
  size_t bytes_read = jack_ringbuffer_read(Rb_val(_ringbuffer),
                                           (char *)destination, byte_count);
  return Val_int((int)(bytes_read / sizeof(float)));
}

/* Write from a float32 Bigarray into ringbuffer. Offsets and count in samples.
 */
CAMLprim value caml_jack_ringbuffer_write_ba(value _ringbuffer, value _bigarray,
                                             value _offset, value _count) {
  const float *source =
      (const float *)Caml_ba_data_val(_bigarray) + Int_val(_offset);
  size_t byte_count = (size_t)Int_val(_count) * sizeof(float);
  size_t bytes_written = jack_ringbuffer_write(
      Rb_val(_ringbuffer), (const char *)source, byte_count);
...
CAMLprim value caml_jack_ringbuffer_read_array(value _ringbuffer, value _array,
  ...
  int to_read = count < first_segment_available + second_segment_available
                    ? count
                    : first_segment_available + second_segment_available;

  float *first_segment_source = (float *)segments[0].buf;
  float *second_segment_source = (float *)segments[1].buf;

  int read_from_first =
      to_read < first_segment_available ? to_read : first_segment_available;
  ...
  for (int i = read_from_first; i < to_read; i++)
    Store_double_field(
        _array, offset + i,
        (double)second_segment_source[i - first_segment_available]);
...
CAMLprim value caml_jack_ringbuffer_write_array(value _ringbuffer, value _array,
  ...
  int to_write = count < first_segment_space + second_segment_space
                     ? count
                     : first_segment_space + second_segment_space;

  float *first_segment_dest = (float *)segments[0].buf;
  float *second_segment_dest = (float *)segments[1].buf;

  int write_to_first =
      to_write < first_segment_space ? to_write : first_segment_space;
  ...
  for (int i = write_to_first; i < to_write; i++)
    second_segment_dest[i - first_segment_space] =
        (float)Double_field(_array, offset + i);

  jack_ringbuffer_write_advance(ringbuffer, (size_t)to_write * sizeof(float));
  return Val_int(to_write);
}

CAMLprim value caml_jack_ringbuffer_read_advance(value _ringbuffer,
                                                 value _count) {
...
static struct custom_operations sem_ops = {
    "liquidsoap_jack_sem",      sem_block_finalize,
    custom_compare_default,     custom_hash_default,
    custom_serialize_default,   custom_deserialize_default,
    custom_compare_ext_default, custom_fixed_length_default};
...
CAMLprim value caml_jack_sem_create(value _unit) {
  ...
  if (!semaphore)
    caml_failwith("jack_sem_create: out of memory");

...
let () =
  ...
  let c = create "lo-detect" in
  let available, cflags, libs =
    if is_excluded "lo" then (false, [], [])
    else (
      match Pkg_config.get c with
        | None -> (false, [], [])
        | Some pc -> (
            match Pkg_config.query pc ~package:"liblo" with
              | None -> (false, [], [])
              | Some conf -> (true, conf.cflags, conf.libs)))
  in
  write_bool "lo_available" available;
  write_sexp "lo_c_flags.sexp" cflags;
  write_sexp "lo_c_library_flags.sexp" libs

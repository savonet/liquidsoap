...
let () =
  match Array.to_list Sys.argv |> List.tl with
    | "--context" :: context_name :: _ ->
        ...
        let c = create "fdkaac-detect" in
        let available, cflags, libs =
          if is_excluded "fdkaac" then (false, [], [])
          else (
            match Pkg_config.get c with
              | None -> (false, [], [])
              | Some pc -> (
                  match Pkg_config.query pc ~package:"fdk-aac" with
                    | None -> (false, [], [])
                    | Some conf -> (true, conf.cflags, conf.libs)))
        in
        write_bool "fdkaac_available" available;
        write_sexp "fdkaac_c_flags.sexp" cflags;
        write_sexp "fdkaac_c_library_flags.sexp" libs
    | _ ->

  ...
  match Array.to_list Sys.argv |> List.tl with
    | "--context" :: context_name :: _ ->
        ...
        let available, cflags, libs =
          if is_excluded "ao" then (false, [], [])
          else (
            match Pkg_config.get c with
              | None -> (false, [], [])
              | Some pc -> (
                  match Pkg_config.query pc ~package:"ao" with
                    | None -> (false, [], [])
                    | Some conf -> (true, conf.cflags, conf.libs)))
        in
        write_bool "ao_available" available;
        write_sexp "ao_c_flags.sexp" cflags;
        write_sexp "ao_c_library_flags.sexp" libs
    | _ ->

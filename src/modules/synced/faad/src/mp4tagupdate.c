...
unsigned membuffer_write_int24(membuffer *buf, uint32_t data) {
  uint8_t temp[3] = {(uint8_t)(data >> 16), (uint8_t)(data >> 8),
                     (uint8_t)data};
...
void *membuffer_get_ptr(const membuffer *buf) { return buf->data; }

unsigned membuffer_get_size(const membuffer *buf) { return buf->written; }

unsigned membuffer_error(const membuffer *buf) { return buf->error; }
...
unsigned membuffer_transfer_from_file(membuffer *buf, mp4ff_t *src,
  ...
  if ((unsigned)mp4ff_read_data(src, (int8_t *)bufptr + oldsize, bytes) !=
      bytes) {
...
static const char *
find_standard_meta(const char *name) // returns atom name if found, 0 if not
{
  unsigned n;
...
static void membuffer_write_track_tag(membuffer *buf, const char *name,
  ...
  membuffer_write_int32(buf, 8 /*atom header*/ + 8 /*data atom header*/ +
                                 8 /*flags + reserved*/ + 8 /*actual data*/);
  membuffer_write_atom_name(buf, name);
  membuffer_write_int32(buf, 8 /*data atom header*/ + 8 /*flags + reserved*/ +
                                 8 /*actual data*/);
...
static void membuffer_write_custom_tag(membuffer *buf, const char *name,
                                       const char *value) {
  membuffer_write_int32(buf, 8 /*atom header*/ + 0x1C /*weirdo itunes atom*/ +
                                 12 /*name atom header*/ + strlen(name) +
                                 16 /*data atom header + flags*/ +
                                 strlen(value));
...
static uint32_t myatoi(const char *param) { return param ? atoi(param) : 0; }
...
static uint32_t find_atom_v2(mp4ff_t *f, uint64_t base, uint32_t size,
                             const char *name, uint32_t extraheaders,
                             const char *name_inside) {
  uint64_t first_base = (uint64_t)(-1);
  while (
      find_atom(f, base, size,
                name)) // try to find atom <name> with atom <name_inside> in it
  {
    ...
    if (find_atom(f, mybase + (8 + extraheaders), mysize - (8 + extraheaders),
                  name_inside)) {
...
static uint32_t modify_moov(mp4ff_t *f, const mp4ff_metadata_t *data,
  ...
  if (!find_atom_v2(f, total_base, total_size, "udta", 0, "meta")) {
    ...
    *(uint32_t *)p_out = fix_byte_order_32(mp4ff_read_int32(f) + size_delta);
    ...
    mp4ff_read_data(
        f, p_out,
        (uint32_t)(total_size - (ilst_offset - total_base) - ilst_size));

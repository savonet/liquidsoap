...
static int32_t TrackToString(char **str, const uint16_t track,
                             const uint16_t totalTracks) {
  char temp[32];
...
static int32_t mp4ff_set_metadata_name(mp4ff_t *f, const uint8_t atom_type,
                                       char **name) {
  static char *tag_names[] = {"unknown",      "title",
                              "artist",       "writer",
                              "album",        "date",
                              "tool",         "comment",
                              "genre",        "track",
  ...
  while (sumsize < size && !f->stream->read_error) /* CVE-2017-9222 */
  {
    ...
    if (!done) {
      if (atom_type == ATOM_DATA) {
        ...
        } else if (parent_atom_type == ATOM_TRACK ||
                   parent_atom_type == ATOM_DISC) {
          /* if (!done && subsize - header_size >= 8 + 8) */
          /* modified by AJS */
          if (!done && (subsize - header_size) >=
                           (sizeof(char) + sizeof(uint8_t) * 3 +
                            sizeof(uint32_t) + /* version + flags + reserved */
                            +(parent_atom_type == ATOM_TRACK
                                  ? sizeof(uint16_t)
                                  : 0) /* leading uint16_t if ATOM_TRACK */
            ...
            mp4ff_tag_add_field(
                &(f->tags), parent_atom_type == ATOM_TRACK ? "track" : "disc",
                temp, -1);
            if (total > 0) {
              sprintf(temp, "%d", total);
              mp4ff_tag_add_field(&(f->tags),
                                  parent_atom_type == ATOM_TRACK ? "totaltracks"
                                                                 : "totaldiscs",
                                  temp, -1);
            }
            done = 1;
...
static int32_t mp4ff_meta_find_by_name_and_return_len(const mp4ff_t *f,
                                                      const char *item,
                                                      char **value) {
  uint32_t i;
...
int32_t mp4ff_meta_get_num_items(const mp4ff_t *f) { return f->tags.count; }

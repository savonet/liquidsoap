...
let () =
  match Array.to_list Sys.argv |> List.tl with
    | "--context" :: context_name :: _ ->
        ...
        let c = create "samplerate-detect" in
        let available, cflags, libs =
          if is_excluded "samplerate" then (false, [], [])
          else (
            match Pkg_config.get c with
              | None -> (false, [], [])
              | Some pc -> (
                  match Pkg_config.query pc ~package:"samplerate" with
                    | None -> (false, [], [])
                    | Some conf -> (true, conf.cflags, conf.libs)))
        in
        write_bool "samplerate_available" available;
        write_sexp "samplerate_c_flags.sexp" cflags;
        write_sexp "samplerate_c_library_flags.sexp" libs
    | _ ->

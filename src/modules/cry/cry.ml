...
let unix_socket transport fd : socket =
  object
    ...
    method write = Unix_utils.write fd
...
let connect_sockaddr ?bind_address ?timeout sockaddr =
  ...
  (try
   ...
   with exn ->
     ...
     begin try Unix.close socket with _ -> ()
     end;
...
let connect_icy c socket source =
  ...
  try
    ...
    in
    begin try Scanf.sscanf (Bytes.to_string (List.hd ret)) "%[^\r^\n]" f with
      | Scanf.Scan_failure s -> raise (Error (Bad_answer (Some s)))
      | Error _ as e -> raise e
      | _ -> raise (Error (Bad_answer None))
    end;
    ...
    begin try Scanf.sscanf (Bytes.to_string ret) "icy-caps:%[1]" f

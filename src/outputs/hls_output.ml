...
class hls_output p =
  ...
  object (self)
    inherit
      Output.encoded
        ~infallible ~on_start ~on_stop ~autostart ~output_kind:"output.file"
          ~name ~content_kind:kind source as output
    ...
    method private write_playlists =
      ...
      List.iter
        (fun s ->
          if List.length segments > 0 then begin
    ...
    method wake_up activation =
      ...
      if persist && Sys.file_exists persist_at then begin
    ...
    method encode frame ofs len =
      if current_segment.len + len > segment_master_duration then begin
        self#push_current_segment;
        self#new_segment
      end;

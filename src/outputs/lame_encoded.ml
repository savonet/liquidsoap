...
let create_encoder ~samplerate ~quality ~stereo =
...
class to_file
  ~filename ~samplerate ~quality ~stereo ~autostart source =
object (self)
  inherit
    [Lame.encoder] Output.encoded
         ~name:filename ~kind:"output.mp3" ~autostart source

  method reset_encoder encoder m = ""
  method encode e b start len = Lame.encode_buffer_part e b start (len/4)

  val mutable fd = None

  method output_start =
    assert (fd = None) ;
    let enc = create_encoder ~quality ~stereo ~samplerate in
    ...
    (fun p ->
       ...
       let source = Lang.assoc "" 2 p in
         ((new to_file ~filename
             ~quality ~samplerate ~stereo ~autostart source):>source))
...
let proto =
  ...
  [ "mount", Lang.string_t, Some (Lang.string no_mount), None ;
    ...
    "samplerate", Lang.int_t, Some (Lang.int 0), None;
...
object (self)
  inherit
    [Lame.encoder] Icecast2.output ~format:Shout.Format_mp3
  ...
  method output_start =
    ...
    let enc = create_encoder ~samplerate ~quality ~stereo in
      encoder <- Some enc

end

let _ =
    Lang.add_operator "output.icecast.mp3"
      ~descr:
  "Output the source's stream to an icecast2 compatible server in MP3 format."
      proto
      (fun p -> ((new to_shout p):>Types.source))

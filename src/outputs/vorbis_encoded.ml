...
class to_file filename bitrate freq stereo source autostart =
object (self)
  inherit
  ...
  method output_start =
    ...
    let enc,first_header =
      Vorbis.create_encoder
        (get_ogg_enc_params ~stereo ~freq bitrate)
  ...
  Lang.add_operator "output.ogg"
    ...
    (fun p ->
       ...
       let source = Lang.assoc "" 2 p in
         ((new to_file name bitrate freq stereo source autostart):>source))

           ...
           [ ("mount", Lang.string_t, None, None);
            ...
            least this length."));
             ...
             ("on_disconnect",
              ...
              (Some
                 (Lang.val_cst_fun [ ("", Lang.string_t, None) ] Lang.unit)),
      ...
      let get_meta meta =
        ...
        in
          (Bytes.set ret 0 (Char.chr pad);
           ...
           let ret = Bytes.unsafe_to_string ret
           in
             if ret <> c.latest_meta
             then (c.latest_meta <- ret; ret)
             else "\000") in
               ...
               (fun () ->
                  let __pa_duppy_0 =
                    Duppy.Monad.Io.exec ~priority: Tutils.Maybe_blocking
                      c.handler
                      (let ret =
                         Tutils.mutexify c.mutex (fun () -> c.state) ()

  ...
  let feed_from_frame t frame =
    ...
    let rec feed_all ofs = function
      | (cstop, content) :: contents ->
          ...
          ( match mode t with
            (* TODO: generators only works for raw data for now *)
            | `Audio ->
                put_audio t (Frame.get_raw content.Frame.audio) ofs cstop
            | `Video ->
                put_video t (Frame.get_raw content.Frame.video) ofs cstop
            | `Both ->
                put_audio t (Frame.get_raw content.Frame.audio) ofs cstop;
                put_video t (Frame.get_raw content.Frame.video) ofs cstop
            | `Undefined -> () );
    ...
    let rec blit audio video =
      match (audio, video) with
        ...
        | (ablk, apos, apos', al) :: audio, (vblk, vpos, vpos', vl) :: video ->
            ...
            let ctype =
              {
                Frame.audio = `Raw (Array.length ablk);
                video = `Raw (Array.length vblk);
                midi = `Raw 0;
              }
            in

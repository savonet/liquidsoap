    ...
    value _dst, value _dst_off)
{
  ...
  /* Drop a few samples to feet into the output buffer doesn't hurt that much. */
  if (dst_off + newlen > dst_len)
    newlen = dst_len - dst_off ;
  ...
  //      from macros.
#define s2f(x) (((double)x)/SHRT_MAX)
#if BYTE_ORDER != LITTLE_ENDIAN
...
CAMLprim value caml_float_pcm_from_s16le(value a, value _aoffs, value _buf, value _boffs, value _len)
{
  ...
  int len = Int_val(_len) / 4;
  int i, c;

  if (Wosize_val(a) < 2)
    caml_invalid_argument("from_s16le: not enough channels");
  ...
  for(c = 0; c < 2; c++)
  {
    cbuf = Field(a, c);
      for(i = 0; i < len; i++)
        Store_double_field(cbuf, i + aoffs, ((double)buf[2*i+c+boffs])/32768);
  }

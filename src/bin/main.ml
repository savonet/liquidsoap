...
let load_libs =
  ...
  fun () ->
    if !should_load_libs && not !loaded then (
  ...
  ref
    ...
    @ [
        ...
        ( ["--libs-locations"],
          Arg.Unit
            (fun () ->
              Printf.printf "Default libraries are installed at:\n%s\n"
                (String.concat ", " Liquidsoap_sites.Sites.libs);
              exit 0),
          "List the location of the default libraries" );
        ( ["--no-libs"],
          Arg.Clear should_load_libs,
          Printf.sprintf "Do not load default libraries." );
        ( ["--no-deprecated"],

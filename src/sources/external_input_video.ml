      ...
      let read_header read =
        ...
        let check = function
          | `Video (fmt, w, h, fps) ->
              ...
              log#info "Format: %s."
                (match fmt with `RGB24 -> "RGB24" | `I420 -> "YUV420");
              ...
              let converter =
                let conv =
                  let pix =
                    match fmt with
                      | `RGB24 ->
                          Image.Generic.Pixel.RGB Image.Generic.Pixel.RGBA32
                      | `I420 ->
                          Image.Generic.Pixel.YUV Image.Generic.Pixel.YUVJ420
                  in
                  Video_converter.find_converter pix
                    (Image.Generic.Pixel.RGB Image.Generic.Pixel.RGBA32)
                in
                fun data ->
                  ...
                  if
                    ...
                    && video_format = `RGB24
                  then Image.Generic.to_RGBA32 src
                  else (
      ...
      let on_data abg reader =
        match Avi.Read.chunk reader with
          ...
          | `Frame (`Audio, _, data) ->
              ...
              else Generator.put_audio abg data 0 (Array.length data.(0))

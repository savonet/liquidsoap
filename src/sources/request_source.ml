...
class virtual unqueued ~kind ~name = object (self)
          ...
          | None ->
            self#log#severe
              "Finished with a non-existent file?! Something may have been \
               moved or destroyed during decoding. It is VERY dangerous, avoid \
               it!"
...
let queued_proto =
  [ "length", Lang.float_t, Some (Lang.float 40.),
    ...
    Some "If true, estimated remaining time on the current track is not \

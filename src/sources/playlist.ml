...
class virtual vplaylist reload random playlist_uri timeout =
object (self)
  ...
  method load_playlist ?(uri=playlist_uri) reload =
    let _playlist =
        ...
        if is_dir filename then begin
          ...
          try
            ...
            let content = String.create length in
              ...
              let (format,playlist) = Playlist_parser.search_valid content in
      ...
      in
        ...
        match Request.create ~audio:false uri with
          | None ->
              self#log#f 2 "Could not resolve playlist URI %S!" uri ;
              []
          | Some req ->
              if Request.Resolved = Request.resolve req timeout then
                let l =
                ...
                self#log#f 2 "Timeout when resolving playlist URI %S!" uri ;
                Request.destroy req ;
                []
              end
    in
      if _playlist <> [] || not reload then begin
...
object
  ...
  inherit vplaylist reload_meth rand_meth uri timeout as pl
...
(** Safe playlist, without queue and playing only local files,
  * which never fails. *)
class safe_playlist reload_meth rand_meth local_playlist timeout = 
object (self)

  inherit vplaylist reload_meth rand_meth local_playlist timeout as pl
      ...
      (fun params ->
         let reload,random,timeout,uri =
           let e v = List.assoc v params in
             ...
             (Lang.to_string (e ""))
         in
           ((new safe_playlist reload random uri timeout):>source))

      ...
      (List.map
         ...
         (Array.to_list
            (
              ...
              with
                | Sys_error _ ->
                    log 3 ("Could not read directory " ^ dir);
                    [||]
            )
         )
      )
...
object (self)
  ...
  (** Randomly exchange files in the playlist.
    * We use the perfect Fisher-Yates shuffle
    ...
    let _playlist =
      let read_playlist filename =
        if is_dir filename then begin
          self#log 3 "Playlist is a directory" ;
          List.filter self#is_valid (list_files self#log filename)
        end else
          (* Parsing the playlist.
           * TODO support more formats thanks to http_playlist *)
                ...
                let acc =
                  if line.[0] = '#' then acc else
                    if self#is_valid line then line::acc else begin
                      self#log 2 (Printf.sprintf "Invalid line %d" n) ;
                      acc
                    end
                in
      ...
      in
        ...
        match Request.create ~audio:false uri with
          | None ->
              self#log 2
                (Printf.sprintf "Could not resolve playlist URI %S!" uri) ;
              []
          | Some req ->
              if Request.Resolved = Request.resolve req 10. then
                ...
                self#log 2
                  (Printf.sprintf "Timeout when resolving playlist URI %S!"
                     uri) ;
                Request.destroy req ;
                []
              end

...
object (self)
  ...
  method virtual stype : Types.source_t
      ...
      (fun s ->
         ...
         in
           Array.fold_left
             ...
             (List.fold_left
                (fun s r ->
                   match Request.get_metadata r "status" with
                   | Some status ->
                       begin match Request.get_metadata r "uri" with
                       | Some uri ->
                           s^(Printf.sprintf "\n[%s] %s" status uri)
                       | None -> s
                       end
  ...
  method get_next n =
    ...
    | Randomize ->
        let a = Array.make n "" in
          for i = 1 to min n (Array.length !playlist)-index_played-1 do
            a.(i-1) <- !playlist.((index_played+i) mod (Array.length !playlist))
          done ;
          Mutex.unlock mylock ;
          a

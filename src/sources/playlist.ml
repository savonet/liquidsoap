...
let is_dir f = try (Unix.stat f).Unix.st_kind = Unix.S_DIR with _ -> false
...
class virtual vplaylist ~on_track ~mime ~reload ~random ~timeout ~prefix
  playlist_uri =
  object (self)
    ...
    self#register_command "reload"
      ~descr:"Reload the playlist, unless already being loaded." (fun _ ->
    ...
    self#register_command "uri"
      ~descr:
        "Print playlist URI if called without an argument, otherwise set a new \
         one and load it."
      ~usage:"uri [<URI>]" (fun s ->
        if s = "" then playlist_uri
        else (
          self#reload_playlist ~uri:s `Other;
          "OK" ));
    self#register_command "next"
      ...
      (fun s ->
        let n = try int_of_string s with _ -> 10 in
        Array.fold_left
          (fun s uri -> s ^ uri ^ "\n")
          (List.fold_left
             (fun s r ->
               let get s =
                 match Request.get_metadata r s with Some s -> s | None -> "?"
               in
               Printf.sprintf "[%s] %s\n" (get "status") (get "initial_uri") ^ s)
             "" self#copy_queue)
          (self#get_next n))

    method virtual private is_valid : string -> bool

    method virtual private check_next : Request.t -> bool
    ...
    * For round-based reloads, already loaded files won't be expired. *)
    method load_playlist ?(uri = playlist_uri)
        (reload : [ `No | `Round | `Other ]) =
      let _playlist =
        let read_playlist filename =
          if is_dir filename then begin
            ...
            try
              ...
              let pwd =
                if Http.is_url uri then Http.dirname uri
              ...
              let format, playlist =
                match mime with
                  ...
                  | x -> (
                      match Playlist_parser.parsers#get x with
                        | Some plugin ->
                ...
                (fun (l, x) ->
                  if l = [] then x
                    ...
                    Printf.sprintf "annotate:%s:%s"
                      (String.concat ","
                         (List.map (fun (l, v) -> Printf.sprintf "%s=%S" l v) l))
                      x)
                playlist
            with e ->
              self#log#important "Could not parse playlist: %s"
        ...
        match Request.resolve req timeout with
          ...
          | e ->
              ...
              in
              self#log#severe "%s when resolving playlist URI %S!" reason uri;
              Request.destroy req;
              []
      in
      ...
      if _playlist = [] && reload <> `No && self#stype = Infallible then
        ...
        self#log#important "Successfully loaded a playlist of %d tracks."
    ...
    * rid of reload overlaps, which requires more duppy synchronization. *)
    method private may_autoreload ~force_reload round_done =
      ...
      match (force_reload, reload) with
        | true, _ -> self#reload_playlist `Other
        | _, Never | _, Watch -> ()
        | _, Every_N_seconds n ->
        ...
        let pos, uri =
          match random with
            | Randomize ->
                ...
                let last = index_played >= Array.length !playlist - 1 in
                let force_reload = on_track ~last pos in
            ...
            | Random ->
                ...
                let force_reload = on_track ~last:false index_played in
    ...
    method get_next =
      Tutils.mutexify mylock (fun n ->
          match random with
            | Normal ->
                Array.init n (fun i ->
                    !playlist.((index_played + i + 1) mod Array.length !playlist))
            | Randomize ->
                Array.init
...
class playlist ~kind ~on_track ~mime ~reload ~random ~check_next ~length
  ~default_duration ~timeout ~prefix ~conservative uri =
  object (self)
    ...
    inherit
      Request_source.queued
        ~kind ~name:"playlist" ~length ~default_duration ~timeout ~conservative
          () as super
...
(** Safe playlist, without queue and playing only local files,
  * which never fails. *)
class safe_playlist ~kind ~on_track ~mime ~reload ~random ~prefix local_playlist
  =
  object (self)
    inherit
      vplaylist
        ~on_track ~mime ~reload ~random ~timeout:10. ~prefix local_playlist as pl
    ...
    * thus, we can assume that the source is infallible. *)
    ...
    method load_playlist ?uri reload =
      ...
      if Array.length !playlist = 0 then failwith "Empty default playlist !"
...
let () =
  let proto =
    [
      ( "mode",
        ...
        Some
          "Play the files in the playlist either in the order (\"normal\" \
           mode), or shuffle the playlist each time it is loaded, and play it \
           in this order for a whole round (\"randomize\" mode), or pick a \
           random file in the playlist each time (\"random\" mode)." );
      ( "reload",
        Lang.int_t,
        Some (Lang.int 0),
        Some
          "Amount of time (in seconds or rounds), when applicable, before \
           which the playlist is reloaded; 0 means never." );
      ( "reload_mode",
        Lang.string_t,
        Some (Lang.string "seconds"),
        Some
          "Unit of the reload parameter, either 'rounds', 'seconds' or 'watch' \
           (reload the file whenever it is changed)." );
      ( "mime_type",
        Lang.string_t,
        Some (Lang.string ""),
        Some
          "Default MIME type for the playlist. Empty string means automatic \
           detection." );
      ( "on_track",
        Lang.fun_t
          [(false, "last", Lang.bool_t); (false, "", Lang.int_t)]
          Lang.bool_t,
        Some
          (Lang.val_cst_fun
             [("", Lang.int_t, None); ("last", Lang.bool_t, None)]
             (Lang.bool false)),
        Some
          "Function to execute when playlist is about to play its next track. \
           Receives track position in the playlist and wether this is the last \
           track. Force a reload by returning `true` in this function. " );
      ( "prefix",
        Lang.string_t,
        Some (Lang.string ""),
        Some
          "Add a constant prefix to all requests. Useful for passing extra \
           information using annotate, or for resolution through a particular \
           protocol, such as replaygain." );
      ("", Lang.string_t, None, Some "URI where to find the playlist.");
    ]
  in
  let reload_of i s =
  ...
  let check_next k =
    ( "check_next",
      ...
      Some
        "Function used to filter next tracks. A candidate track is only \
         validated if the function returns true on it. The function is called \
         ...
         repetitions, but be careful: if the function rejects all attempts, \
         the playlist will enter into a consuming loop and stop playing \
         anything." )
  in

  let fmt = Lang.univ_t 1 in

  Lang.add_operator "playlist" ~category:Lang.Input
    ...
    (fun params kind ->
      ...
      in
      ( new playlist
          ~kind ~mime ~reload ~random ~on_track ~length ~default_duration
          ~prefix ~timeout ~check_next ~conservative uri
        :> source ));

  Lang.add_operator "playlist.safe" ~category:Lang.Input
    ~descr:
      "Loop on a playlist of local files, and never fail. In order to do so, \
       it has to check every file at the loading, so the streamer startup may \
       take a few seconds. To avoid this, use a standard playlist, and put \
       only a few local files in a default safe_playlist in order to ensure \
      ...
      let on_track ~last pos =
        Lang.to_bool
          (Lang.apply ~t:Lang.bool_t on_track
             [("last", Lang.bool last); ("", Lang.int pos)])
      in
      ( new safe_playlist ~kind ~mime ~reload ~prefix ~on_track ~random uri
        :> source ))

...
object (self)
  ...
  initializer
    for i = 0 to nb_blocks - 1 do
      buffer.(i) <- Array.init buffer_chans (fun _ -> Array.make buffer_length 0.)
    done
  val mutable read = 0
  val mutable write = 0
    ...
    let params = Pcm.get_params dev in
      ...
      let fill block =
        ...
        with
          ...
          | Buffer_xrun ->
              (* Restart. Try again ... *)
              Dtools.Log.log ~label:"input.alsa" 2 "Overrun!" ;
              Pcm.prepare dev
      in
  ...
  method get_frame buf =
    ...
    let buffer =
      ...
      if write = read then begin
        Dtools.Log.log ~label:"input.alsa" 2 "No available frame!" ;
        blank
      end else

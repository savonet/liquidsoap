...
let rec pop_indicator t =
  ...
  in
    if i.temporary then
      ...
      with
        | e ->
            Log.log ~label:"request" 2
              (Log.f "Unlink failed: %S" (Printexc.to_string e))
      end ;
    t.decoder <- None ;
...
let push_indicators t l =
  if l <> [] then
    let hd = List.hd l in
      log t (Printf.sprintf "Pushed [%S;...]." hd.string) ;
...
let get_all_metadata t =
  let h = Hashtbl.create 20 in
    ...
    (* Add builtin information *)
    Hashtbl.replace h "rid" (string_of_int t.id) ;
    Hashtbl.replace h "initial_uri" t.initial_uri ;
    Hashtbl.replace h "temporary"
      (match t.indicators with
         | (h::_)::_ -> if h.temporary then "true" else "false"
         | _ -> "false") ;
    begin match t.resolving with
      | Some d ->
          Hashtbl.replace h "resolving" (pretty_date (Unix.localtime d))
      | None -> ()
    end ;
    begin match t.on_air with
      | Some d ->
          Hashtbl.replace h "on_air" (pretty_date (Unix.localtime d))
      | None -> ()
    end ;
    Hashtbl.replace h "status"
      (match t.status with
         | Idle -> "idle"
         | Resolving -> "resolving"
         | Ready -> "ready"
    ...
    begin match get_filename t with
      | Some f -> Hashtbl.replace h "filename" f
      | None -> ()
    end ;
    h
...
let max = Conf.ref_int ~default:Configure.requests_max_id "max_rid"
...
let create ?(metadata=[]) ?(audio=true) ?(persistent=false) ?(indicators=[]) u =
  ...
  let rid = next () in
    if rid = -1 then
      ( Mutex.unlock lock ;
	Log.log ~label:"request" 2 "No available RID !" ;
	None )
    else
      let t = {
        id = rid ;
...
let resolve t timeout =
  ...
  let resolve_step () =
    ...
    let proto,arg = parse_uri i.string in
      match protocols#get proto with
        | Some handler ->
            log t (Printf.sprintf
                     "Resolving %S (timeout %.fs)..." i.string timeout) ;
            let production = handler.resolve ~log:(log t) arg maxtime in
              if production = [] then
                ignore (pop_indicator t)
              else
                push_indicators t production
        | None ->
            log t "Unknown protocol !" ;
            pop_indicator t
  in
  let result =
    try

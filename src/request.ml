...
let mresolvers =
  Plug.create ~doc:mresolvers_doc ~insensitive:true "metadata resolvers"
...
let () = Var.register "max_rid" Var.Int
        ...
        | Some handler ->
            ...
            let production = handler.resolve ~log:(log t) arg maxtime in
              if production = [] then begin
                Dtools.Log.log 4 ~label:"request"
                  (Dtools.Log.f "Failed to resolve %S! See request's log (telnet command 'trace %d' for more information." i.string t.id) ;

...
let create_decoder ?(merge_tracks = false) _ ~width ~height ~channels ~mode
  ...
  let decode buffer =
    ...
    if decode_audio then (
      ...
      let samplerate = SyncLazy.force Frame.audio_rate in
    ...
    if decode_video then (
      ...
      let fps = { Decoder.num = SyncLazy.force Frame.video_rate; den = 1 } in
    ...
    ~doc:"Decode a file or stream using GStreamer."
    {
      ...
      file_type =
        (fun ~ctype:_ filename ->
          let channels = SyncLazy.force Frame.audio_channels in

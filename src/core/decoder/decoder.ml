...
let channel_layout audio =
  Multicore.force Content.(Audio.(get_params audio).Content.channel_layout)
          ...
          let video_scale =
            let width, height =
              ...
              with Content.Invalid ->
                ...
                ( Multicore.force Frame.video_width,
                  Multicore.force Frame.video_height )
            in
            Decoder_utils.video_scale ~width ~height ()
          in
          let out_freq =
            Decoder_utils.{ num = Multicore.force Frame.video_rate; den = 1 }
          in

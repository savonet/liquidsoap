...
let _ =
  ...
  let fill b =
    Lang.val_fun
      [("", "", None)]
      (fun p ->
        let c = List.assoc "" p |> Lang.to_int |> Video_color.yuv_of_int in
  ...
  let line_to board =
    Lang.val_fun
      ...
      (fun p ->
        let r, g, b =
          List.assoc "c" p |> Lang.to_int |> Video_color.rgb_of_int
        in
        let x = List.assoc "x" p |> Lang.to_int in
        let y = List.assoc "y" p |> Lang.to_int in
      ...
      (fun p ->
        ...
        Lang.reference
          (fun () ->
            ...
            Lang.int c)
          (fun c ->
            let r, g, b = c |> Lang.to_int |> Video_color.rgb_of_int in

  ...
  ignore
    (Lang.add_operator ~base:Modules.synth_all name
       ...
       (fun p ->
         ...
         in
         let synths =
           Array.init (Multicore.force Frame.midi_channels) (fun c ->
...
let () =
  register
    (fun adsr -> new Synth.sine ?adsr (Multicore.force Frame.audio_rate))
    "sine" "Sine synthesizer."

let () =
  register
    (fun adsr -> new Synth.square ?adsr (Multicore.force Frame.audio_rate))
    "square" "Square synthesizer."

let () =
  register
    (fun adsr -> new Synth.saw ?adsr (Multicore.force Frame.audio_rate))
    "saw" "Saw synthesizer."

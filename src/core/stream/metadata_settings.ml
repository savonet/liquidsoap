...
(* Metadata export settings. They live under settings.encoder.metadata.* and
   are read both when exporting metadata to an encoder and when resolving a
   request, i.e. from below the encoder. Defined here so that neither of those
   has to depend on the encoders. *)

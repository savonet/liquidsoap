...
module VideoSpecs = struct
  ...
  let merge p p' =
    {
      width =
        Option.map Lazy.from_val
          (Content.merge_param ~name:"width"
             (Option.map Lazy.force p.width, Option.map Lazy.force p'.width));
      height =
        Option.map Lazy.from_val
          (Content.merge_param ~name:"height"
  ...
  let fill src src_pos dst dst_pos len =
    ...
    for i = 0 to !len do
      Video.Canvas.set dst (dst_pos + i) (Video.Canvas.get src (src_pos + i))
    done
...
module MidiSpecs = struct
  ...
  let length m = Frame_base.main_of_midi (MIDI.Multitrack.duration m)
...
module Midi = struct
  include Content.MkContent (MidiSpecs)
...
let default_audio () =
  ...
  if channels = 0 then Content.None.format
  else Audio.format_of_channels channels

let default_video () =
  if Lazy.force Frame_base.default_video_enabled then
    Video.lift_params
      {
        width = Some Frame_base.video_width;
        height = Some Frame_base.video_height;
      }
  else Content.None.format

let default_midi () =
  let channels = Lazy.force Frame_base.midi_channels in
  if channels = 0 then Content.None.format else Midi.lift_params { channels }

...
let encode_audio_frame ~source_idx ~type_t ~mode ~opts ?codec ~format
  ...
  in
  let target_samplerate = SyncLazy.force format.Ffmpeg_format.samplerate in
...
let encode_video_frame ~source_idx ~type_t ~mode ~opts ?codec ~format ~field
    generator =
  let internal_fps = SyncLazy.force Frame.video_rate in
  ...
  let target_fps = SyncLazy.force format.Ffmpeg_format.framerate in
  let target_frame_rate = { Avutil.num = target_fps; den = 1 } in
  let target_width = SyncLazy.force format.Ffmpeg_format.width in
  let target_height = SyncLazy.force format.Ffmpeg_format.height in

  ...
  ignore
    (Lang.add_operator ~base:Modules.synth_all name
       ...
       (fun p ->
         ...
         in
         let synths =
           Array.init (Lazy.Mutexed.force Frame.midi_channels) (fun c ->
...
let () =
  register
    (fun adsr -> new Synth.sine ?adsr (Lazy.Mutexed.force Frame.audio_rate))
    "sine" "Sine synthesizer."

let () =
  register
    (fun adsr -> new Synth.square ?adsr (Lazy.Mutexed.force Frame.audio_rate))
    "square" "Square synthesizer."

let () =
  register
    (fun adsr -> new Synth.saw ?adsr (Lazy.Mutexed.force Frame.audio_rate))
    "saw" "Saw synthesizer."

...
class compress ~field (source : source) attack release threshold ratio knee
  rms_window gain =
...
let compress p =
  ...
  let attack, release, threshold, ratio, knee, rmsw, gain, (field, src) =
...
let _ =
  ...
  Lang.add_track_operator ~base:Compress.audio_compress "old"
    (( "ratio",
       Lang.getter_t Lang.float_t,
       Some (Lang.float 2.),

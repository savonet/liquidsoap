...
(** What a frame gets when a script gives it no type of its own, which core
    reads from the settings. *)
val default_audio_channels : (unit -> int) ref

val default_video : (unit -> bool) ref

(* Same as [Lang.frame_t] (with position) *)
val make : ?pos:Pos.t -> Type.t -> Type.t Fields.t -> Type.t
...
val content_type : Type.t -> Content_base.format Fields.t

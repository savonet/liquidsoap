...
type active_params = {
  ...
  mutable current_sync_source : sync_source option;
  log : Log.t;
...
let _check_self_sync ~clock x =
  ...
  let new_sync_source =
    match self_sync_sources with
      ...
      | _ -> None
  in
  if x.current_sync_source <> new_sync_source && x.sync = `Automatic then (
    (match (x.current_sync_source, new_sync_source) with
      | None, Some s ->
      ...
      | Some _, Some s ->
          x.log#important "Switching self-sync source to %s"
            (string_of_sync_source s)
      | None, None -> ());
    x.current_sync_source <- new_sync_source;
    let time_implementation =

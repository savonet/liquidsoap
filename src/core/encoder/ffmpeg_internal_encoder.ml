...
let mk_audio ~mode ~codec ~params ~options ~field output =
  ...
  let target_samplerate = SyncLazy.force params.Ffmpeg_format.samplerate in
  ...
  let internal_converter () =
    let src_samplerate = SyncLazy.force Frame.audio_rate in
...
let mk_video ~mode ~codec ~params ~options ~field output =
  ...
  let target_fps = SyncLazy.force params.Ffmpeg_format.framerate in
  let target_video_frame_time_base = { Avutil.num = 1; den = target_fps } in
  let target_width = SyncLazy.force params.Ffmpeg_format.width in
  let target_height = SyncLazy.force params.Ffmpeg_format.height in
  ...
  let internal_converter cb =
    let src_width = SyncLazy.force Frame.video_width in
    let src_height = SyncLazy.force Frame.video_height in
    let scaler =

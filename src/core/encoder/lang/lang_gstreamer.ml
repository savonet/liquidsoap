...
let type_of_encoder p =
  let has_video =
    List.exists
      (function
        | `Labelled ("has_video", { Term.term = `Bool true }) -> true
        | _ -> false)
      p
  in

...
let encoder id ext =
  ...
  let header =
    if ext.video <> None then (
      ...
      let width = SyncLazy.force width in
      let height = SyncLazy.force height in
      Avi.header ~width ~height ~channels:ext.channels
        ~samplerate:(SyncLazy.force ext.samplerate)
        ())
    else if ext.header then
      Wav_aiff.wav_header ~channels:ext.channels
        ~sample_rate:(SyncLazy.force ext.samplerate)
        ~sample_size:16 ()
    else ""
  in
  ...
  let ratio =
    float (SyncLazy.force ext.samplerate) /. float (Frame.audio_of_seconds 1.)
  in
  let encode frame start len =
    let channels = ext.channels in
    let samplerate = SyncLazy.force ext.samplerate in
    let sbuf =
      if ext.video <> None then (
        let width, height = Option.get ext.video in
        let width = SyncLazy.force width in

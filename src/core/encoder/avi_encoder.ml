...
let encode_frame ~channels ~samplerate ~width ~height ~converter frame start len
  ...
  let ratio = float samplerate /. float (Multicore.force Frame.audio_rate) in
  let audio =
...
let encoder avi =
  ...
  let samplerate = Multicore.force avi.samplerate in
  let converter = Audio_converter.Samplerate.create channels in
  let width = Multicore.force avi.width in
  let height = Multicore.force avi.height in

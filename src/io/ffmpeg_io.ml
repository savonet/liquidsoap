...
class input ?(name = "input.ffmpeg") ~autostart ~self_sync ~poll_delay ~debug
  ...
  object (self)
    ...
    method self_sync = (`Dynamic, self_sync && self#is_connected)
        ...
        @ [
            ...
            ( "status",
              ...
              "Return the current status of the source, either \"stopped\" \
               ...
               (attempting to connect to the HTTP stream), \"connected <url>\" \
               (connected to <url>, buffering or playing back the stream) or \
               \"stopping\" (source is stopping).",
              fun s ->
                val_fun [] (fun _ ->
                    string

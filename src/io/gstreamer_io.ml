    ...
    if should_run then
      try
        ...
        Tutils.mutexify element_m (fun () ->
          ...
          ignore (Element.get_state el.bin);
          retry_in <- -1.;
          GU.flush ~log:self#log ~on_error:(fun err -> retry_in <- on_error (Flushing_error err)) el.bin) ();
        Tutils.mutexify restart_m (fun () ->
          restarting <- false) ();
        if retry_in >= 0. then
...
let output_proto ~kind ~pipeline = Output.proto @ [
  "clock_safe",
  ...
  "pipeline",
  Lang.string_t, Some (Lang.string pipeline),
...
object (self)
  ...
  initializer
    ...
    self#register_command
      "restart" ~descr:"Restart gstreamer pipeline state to paused" (fun _ ->
        self#restart; "Done. Task will complete asynchronously.")

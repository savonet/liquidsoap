...
let conf_log = Dtools.Conf.void ~p:(conf_srt#plug "log") "Log configuration"
...
class input ~kind ~bind_address ~max ~payload_size ~clock_safe ~on_connect
  ~on_disconnect ~messageapi format =
  ...
  object (self)
    ...
    method private handle_client socket =
      ...
      Tutils.mutexify input_mutex
        ...
        ();
      if clock_safe then self#get_clock#register_blocking_source;
      on_connect ()
        ...
        (fun () ->
          match client_data with
            ...
            | Some (socket, _) ->
                ...
                client_data <- None)
        ();
      if clock_safe then self#get_clock#unregister_blocking_source;
      self#connect
  ...
  Lang.add_operator "input.srt" ~kind:(Lang.Unconstrained kind)
    ...
    (fun p kind ->
      ...
      ( new input
          ...
          ~on_disconnect ~messageapi ~max format
        :> Source.source ))
...
class output ~kind ~payload_size ~messageapi ~on_start ~on_stop ~infallible
  ...
  object (self)
    ...
    method private send_chunks =
      let len = Tutils.mutexify output_mutex (fun () -> Buffer.length buffer) in
    ...
    method private send data =
      if self#is `Connected then begin
        Tutils.mutexify output_mutex (Buffer.add_string buffer) data;
        self#send_chunks
      end
  end

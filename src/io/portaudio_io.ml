...
class input ~kind ~clock_safe ~start ~on_start ~on_stop ~fallible ~device_id
  ~latency buflen =
  object (self)
    inherit base
...
let () =
  ...
  Lang.add_operator "output.portaudio"
    ...
    (fun p ->
      ...
      (new output
         ~kind ~start ~on_start ~on_stop ~infallible ~clock_safe ~device_id
  ...
  Lang.add_operator "input.portaudio"
    ...
    (fun p ->
      ...
      let kind = Kind.of_kind kind in
      new input
        ~kind ~clock_safe ~start ~on_start ~on_stop ~fallible ~device_id
        ~latency buflen)

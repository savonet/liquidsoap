...
object (self)
  ...
  initializer Dtools.Conf.set_bool "root.sync" false
      ...
      let params = Pcm.get_params dev in
        ...
        (
          ...
          with
            | _ ->
                ...
                handle "access" (Pcm.set_access dev params) Pcm.Access_rw_interleaved ;
                ...
                (fun pcm buf ofs len ->
                   ...
                   let r = Pcm.readi pcm sbuf 0 len in
                     Float_pcm.from_s16le buf ofs sbuf 0 (r * 4);
                     r
                )
        );
        ...
        in
          ...
          if bufsize <> samples_per_frame then
            self#log 3
              ("Could not set buffer size to 'frame.size' "^
               Printf.sprintf
                 "(%d samples), got %d."
          ...
          with
            | Alsa.Invalid_argument as e ->
                self#log 1 "Setting alsa parameters failed (invalid argument)!";
                raise e
          );
          Pcm.set_nonblock dev false ;
          pcm <- Some dev
    with

...
object
  ...
  initializer
    ring <- Array.init
      ...
      (fun _ ->
         let r = Ringbuffer.create (ringbuffer_coeff * samples_per_frame) in
  ...
  method get_frame ab =
    ...
    let dest = AFrame.get_float_pcm ab in
      ...
      for chan = 0 to Array.length port_names - 1 do
        ...
        while Ringbuffer.read_space ring.(chan) > (ringbuffer_coeff - 2) * buflen do
          let n = Ringbuffer.read_space ring.(chan) - buflen in
            Ringbuffer.read_advance ring.(chan) n;
            self#log#f 4 "Dropping %d samples from the ringbuffer." n
        done;
...
let () =
  Lang.add_operator "input.jack"
    ...
    ~descr:"Jack input."
    [
      "ports", Lang.list_t Lang.string_t, Some (get_default_ports "input"), Some "Port names." ;
      "synchronize", Lang.bool_t, Some (Lang.bool true), Some "Synchronize on jack input." ;
  ...
  Lang.add_operator "output.jack"
    ...
    ~descr:"Jack output."
    [
      "ports", Lang.list_t Lang.string_t, Some (get_default_ports "output"), Some "Port names." ;
      "synchronize", Lang.bool_t, Some (Lang.bool true), Some "Synchronize on jack output." ;

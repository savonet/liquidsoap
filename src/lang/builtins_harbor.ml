...
struct
  ...
  let () =
    Lang_builtins.add_builtin ("harbor." ^ Harbor.name ^ ".register") ~cat:Liq
      ~descr:(Printf.sprintf 
                "Register a %s handler on the harbor. \
  ...
  let () =
    Lang_builtins.add_builtin ("harbor." ^ Harbor.name ^ ".remove") ~cat:Liq
      ~descr:(Printf.sprintf "Remove a registered %s handler on the harbor." name_up)

...
(** Regular expressions, as a liquidsoap value. The builtins built on top of
    them live in [Builtins_regexp]. *)
...
let regexp_t = RegExp.t
let to_regexp = RegExp.of_value
let regexp = RegExp.to_value ?pos:None
let descr_of_regexp { descr; _ } = descr

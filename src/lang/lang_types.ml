...
let repr ?(filter_out=fun _->false) ?(generalized=[]) t : repr =
  ...
  let rec repr t =
    if filter_out t then `Ellipsis else
      ...
      | Arrow (args,t) ->
         `Arrow (List.map (fun (opt,lbl,t) -> opt,lbl,repr t) args, repr t)
      | EVar (id,c) ->
...
let print_repr f t =
  ...
  let rec print ~par vars : repr -> DS.t = function
    ...
    | `Uple [] -> Format.fprintf f "unit" ; vars
    | `Uple l ->
       if par then
         Format.fprintf f "@[<1>("
       else
...
let rec occur_check a b =
  ...
  | Uple l -> List.iter (occur_check a) l
...
let rec (<:) a b =
  ...
  | Uple l, Uple m ->
     if List.length l <> List.length m then raise (Error (`Uple [`Ellipsis], `Uple [`Ellipsis]));
     List.iter2
       (fun a b ->
         try a <: b with
         | Error (a,b) -> raise (Error (`Uple [`Ellipsis; a; `Ellipsis], `Uple [`Ellipsis; b; `Ellipsis]))
       ) l m
  | Zero, Zero -> ()
  | Zero, Variable -> ()
  | Succ t1, Succ t2 ->
...
let filter_vars f t =
  let rec aux l t =
    ...
    | Uple aa -> List.fold_left aux l aa
    | Constr c ->

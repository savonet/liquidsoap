...
let throw ?(formatter = Format.std_formatter) ?lexbuf () =
  ...
  in
  fun exn bt ->
    let raise exn = Printexc.raise_with_backtrace exn bt in
    match exn with
      (* Warnings *)
          ...
          (if Sys.unix then
             ...
             Format.fprintf formatter
               "Unsupported encoder: %s.@ Please note that, on windows, %%mp3, \
                %%vorbis and many other encoders are not available. Instead, \
                you should use the %%ffmpeg encoder.@]@.")
            fmt;
          raise Error
      ...
      | Runtime_error.(Runtime_error { kind; msg; pos }) ->
          ...
          raise Error
      | Sedlexing.MalFormed ->
          print_error ~formatter 15 "Malformed UTF8 content."

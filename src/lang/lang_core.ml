...
let bigstring i = mk (Ground (Bigstring i))
...
let val_cst_fun p c =
  ...
  match c.value with
    ...
    | Ground (Bigstring i) ->
        f (mkg Type.Ground.string) (`Ground (Term.Ground.Bigstring i))
...
let to_string t =
  match t.value with
    | Ground (String s) -> s
    | Ground (Bigstring bs) -> Bigstringaf.to_string bs
    | _ -> assert false

let to_bigstring t =
  match t.value with
    | Ground (String s) -> bigstring_of_string s
    | Ground (Bigstring bs) -> bs
    | _ -> assert false

let to_string_getter t =
  match t.value with
    | Ground (String s) -> fun () -> s
    | Ground (Bigstring bs) -> fun () -> Bigstringaf.to_string bs
    | Fun _ | FFI _ -> (
        fun () ->
          match (apply t []).value with
            | Ground (String s) -> s
...
let to_bigstring_getter t =
  match t.value with
    | Ground (String s) -> fun () -> bigstring_of_string s
    | Ground (Bigstring bs) -> fun () -> bs
    | Fun _ | FFI _ -> (
        fun () ->
          match (apply t []).value with
            | Ground (String s) -> bigstring_of_string s

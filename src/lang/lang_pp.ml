  ...
  let rec token () =
    ...
    | Lang_parser.PP_IFDEF,_ | Lang_parser.PP_IFNDEF,_ as tok ->
       begin
         match tokenizer() with
         | Lang_parser.VAR v,_ ->
            let test = if fst tok = Lang_parser.PP_IFDEF then fun x -> x else not in
    ...
    | Lang_parser.PP_IFENCODER,_ | Lang_parser.PP_IFNENCODER,_ as tok ->
       ...
       let has_enc = try let (_:Encoder.factory) = Encoder.get_factory fmt in true with Not_found -> false in
       let test = if fst tok = Lang_parser.PP_IFENCODER then fun x -> x else not in
...
let includer dir tokenizer =
  ...
  let peek () =
    try fst3 (Stack.top stack)
    with Stack.Empty -> tokenizer
  in
  let current_dir () =
    try snd3 (Stack.top stack)
...
type exp_item = String of string | Expr of tokenizer | Concat | RPar | LPar | String_of
...
let parse_comments tokenizer =
  let documented_def (doc,doc_startp) (startp,endp) =
    ...
    let rec parse_doc (main,special,params) = function
      ...
      | line::lines ->
         begin try
             let sub = Pcre.exec ~pat:"^\\s*@(category|flag|param)\\s*(.*)$" line in
             ...
             | "param" ->
                ...
                let rec parse_descr descr lines =
                  ...
                  | [] -> raise Not_found
                  | line::lines ->
                     let line = Pcre.substitute ~pat:"^ *" ~subst:(fun _ -> "") line in

...
and in_term =
  ...
  | RFun of Vars.t * (string * string * T.t * term option) list * (unit -> term)
...
let rec map_types f (gen : 'a list) tm =
  ...
  match tm.term with
    ...
    | RFun (fv, p, fn) ->
        let fn () = map_types f gen (fn ()) in
        { t = f gen tm.t; term = RFun (fv, List.map aux p, fn) }
    | Let l ->
...
let rec fold_types f gen x tm =
  ...
  match tm.term with
    ...
    | Fun (_, p, v) -> fold_types f gen (fold_proto x p) v
    | RFun (_, p, fn) -> (
        match (fn ()).term with
          | Let l ->
...
module V = struct
  ...
  and in_value =
    ...
    | Fun of (string * string * value option) list * full_env * full_env * term
...
let rec check ?(print_toplevel = false) ~throw ~level ~env e =
  ...
  match e.term with
    ...
    | Seq (a, b) ->
        ...
        check ~throw ~print_toplevel ~level ~env b;
        e.t >: b.t
    | App (a, l) -> (
        check ~throw ~level ~env a;
        ...
        if print_toplevel then
          add_task (fun () ->
              Format.printf "@[<2>%s :@ %a@]@."
                ...
                def.t);
        check ~throw ~print_toplevel ~level:(level + 1) ~env body;
        e.t >: body.t
...
let check ?(ignored = false) ~throw e =
  ...
  try
    check ~print_toplevel ~throw ~level:(List.length builtins#get_all) ~env:[] e;
...
let rec eval ~env tm =
  ...
  match tm.term with
    ...
    | Let { gen; pat; def = v; body = b; _ } ->
        ...
        let v = eval ~env v in
        let env =
          List.map (fun (x, v) -> (x, (gen, v))) (eval_pat pat v) @ env
        in
        eval ~env b
    | Fun (fv, p, body) ->
        ...
        match (fn ()).term with
          | Let { pat; body } ->
              ...
              let rec ffi args t =
                ...
                let var =
                  match pat with PVar var -> var | _ -> assert false
                in

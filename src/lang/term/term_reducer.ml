...
let set_reducer ~pos ~to_term = function
  | `Set (tm, v) ->
      ...
      `Cast
        {
          casted = mk ~pos (`App (op, [("", to_term v)]));
...
let mk_let_json_parse ~pos (args, pat, def, cast) body =
  ...
  in
  let def = mk ~pos (`Cast { casted = def; typ = ty }) in
...
let mk_let ~env ~pos ~(to_term : env:env -> Parsed_term.t -> Runtime_term.t)
  ...
  match (arglist, decoration) with
    | Some arglist, `None | Some arglist, `Replaces ->
        ...
        let def =
          match cast with
            | Some ty -> mk ~pos (`Cast { casted = def; typ = ty })
            | None -> def
        in
...
let rec to_ast ~env ~pos ast =
  ...
  match ast with
    ...
    | `Null -> `Null
    | `Cast (t, typ) ->
        `Cast { casted = to_term t; typ = Parser_helper.mk_ty ~pos typ }

...
(** Do we have a method. *)
let has_meth a = match a.descr with Meth _ -> true | _ -> false
...
let update_level ~level a =
  ...
  let x = match x.descr with Var x -> x | _ -> assert false in
  occur_check x a
...
let rec ( <: ) =
  ...
  fun a b ->
    ...
    (* TODO: we should use a more efficient data structure *)
    if List.exists (fun (a', b') -> Type.eq a a' && Type.eq b b') !cache then ()
    else (
            ...
            List.iter2
              (fun a b ->
                ...
                with Error (a, b) ->
                  ...
                  let l' =
                    List.init (List.length m - !n) (fun _ -> `Ellipsis)
                  in
                  Printexc.raise_with_backtrace

...
let val_cst_fun p c =
  ...
  let f t tm =
      mk (Fun (p',[],[],{ Term.t = t ; Term.term = tm }))
  in
...
let add_builtin_base ~category ~descr ?(flags=[]) name value t =
  ...
  let value = { pos = t.T.pos ; value = value } in
  let generalized = T.filter_vars (fun _ -> true) t in

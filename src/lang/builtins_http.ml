...
let add_http_request ~stream_body ~descr ~request name =
  ...
  in
  add_builtin name ~cat:Interaction
    ~descr:
      (Printf.sprintf "%s Returns `null` if the connection timed out." descr)
    params (Lang.nullable_t request_return_t) (fun p ->
      ...
      with
        | Curl.(CurlException (CURLE_OPERATION_TIMEOUTED, _, _)) -> Lang.null
        | e ->

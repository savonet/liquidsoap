...
type kind =
  [ `Any | `Internal | `Kind of Lang_content.kind | `Format of Lang_content.format ]
...
(** {2 Lang_content-independent frame operations} *)
...
val audio : t -> Lang_content.data

(** Set a frame's audio content. *)
val set_audio : t -> Lang_content.data -> unit

(** Get a frame's video content. *)
val video : t -> Lang_content.data

(** Set a frame's video content. *)
val set_video : t -> Lang_content.data -> unit

(** Get a frame's midi content. *)
val midi : t -> Lang_content.data

(** Set a frame's midi content. *)
val set_midi : t -> Lang_content.data -> unit

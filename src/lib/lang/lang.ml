...
exception Libs_loaded

let load_libs ?parse_only ?(deprecated = false) () =
  let dirs = Liquidsoap_sites.Sites.libs in
  try
    List.iter
      (fun dir ->
        let file = Filename.concat dir "main.liq" in
        if Sys.file_exists file then (
          ...
          raise Libs_loaded ))
      dirs;
    failwith "Could not find default libraries!"
  with Libs_loaded -> ()

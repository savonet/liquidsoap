...
let () =
  ...
  add_builtin ~cat:Liq "register" ~descr:"Register a new setting."
    ...
    (fun p ->
      ...
      in
      ignore
        (f ~name ~descr ~final_key Utils.conf (Dtools.Conf.path_of_string path));
      Lang.unit)

let () =
  let set cast path v =
    (cast (Utils.conf#path (Dtools.Conf.path_of_string path)))#set v
  in
      ...
      let get_error e =
        match e with
          | Dtools.Conf.Mismatch _ ->
              let t = Utils.conf#path (Dtools.Conf.path_of_string path_s) in
              let kind =
                match t#kind with
...
let () =
  let get cast path v =
    try (cast (Utils.conf#path (Dtools.Conf.path_of_string path)))#get

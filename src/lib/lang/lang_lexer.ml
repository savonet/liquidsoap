...
exception Found_lib of string

let find_lib filename =
  try
    List.iter
      (fun dir ->
        let file = Filename.concat dir filename in
        if Sys.file_exists file then raise (Found_lib file))
      Liquidsoap_sites.Sites.libs;
    raise
      (Lang_values.Runtime_error
         {
           kind = "include";
           msg =
             Some (Printf.sprintf "Could not find default library %s" filename);
           pos = [];
         })
  with Found_lib filename -> filename

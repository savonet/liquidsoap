  ...
  object (self)
    ...
    method private get_frame frame =
      match status with
        ...
        | `Limit ->
            ...
            if source#is_ready then self#get_frame frame
            else
        ...
        | `After ->
            ...
            needs_tick <- true;
            if Generator.length pending_after = 0 && Frame.is_partial frame then begin
              status <- `Idle;
              self#cleanup_transition_source;
    ...
    method private analyze_after =
      ...
      let rec f () =
        ...
        if after_len <= rms_width then begin
          let pcm = content.Frame.audio in
          for i = start to stop - 1 do
            let squares =
              Array.fold_left
    ...
    method remaining =
      let with_buf buf =
        ...
        if rem < 0 then -1 else source#remaining + Generator.length buf
      in
      match status with

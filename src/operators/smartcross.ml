...
(** [rms_width], [inhibit] and [minimum_length] are all in samples.
  ...
  * [f] should become a list associating pairs of
  * (rms_before,rms_after) ranges to transitions. *)
...
object (self)
  ...
  method private buffering n =
    ...
    if AFrame.is_partial b then
      ...
      if n>0 then self#buffering (n - AFrame.position b)
  ...
  method private create_transition =
    ...
    in
    let db_before = Sutils.int_of_float db_before in
    let db_after  = Sutils.int_of_float db_after in
    let before = ((new Generated.consumer gen_before):>source) in
    let after  =
      let beginning =
        ((new Generated.consumer ~metadata_pos:0
                                 ~metadata:after_metadata gen_after):>source)
      in
        ((new Sequence.sequence ~merge:true [beginning;s]):>source)
    in
    let f =
      self#log 3
        (Printf.sprintf "Analysis: %ddB / %ddB (%.2fs / %.2fs)"
           db_before db_after
      ...
      try
        let _,f =
          List.find
            (fun (test,_) ->
              Lang.to_bool (Lang.apply test ["",Lang.int db_before;
                                             "",Lang.int db_after]))
            transitions
        in
          fun a b -> Lang.to_source (Lang.apply f ["",Lang.source a;
                                                   "",Lang.source b])
      with
        | Not_found ->
            self#log 2 "No matching transition!" ;
            fun a b -> ((new Sequence.sequence [a;b]):>source)
    in
    let compound =
      if Generator.length gen_before > minimum_length then
        f before after
      else begin
        self#log 3 "Not enough data for crossing." ;
...
let () =
  Lang.add_operator "smart_cross"
    [ "duration", Lang.float_t, Some (Lang.float 5.),
      ...
      "",
      Lang.list_t (Lang.product_t condition_t transition_t),
      None,
      Some ("List of transition functions (composing from the end of a track "^
            "and the next track) together with conditions indicating which "^
            "transition to use, depending on the power of the signal before "^
    ...
    (fun p ->
       ...
       let transitions = Lang.to_list (Lang.assoc "" 1 p) in
       let transitions = List.map Lang.to_product transitions in

       let conservative = Lang.to_bool (List.assoc "conservative" p) in

       let source = Lang.to_source (Lang.assoc "" 2 p) in
         ((new cross source transitions ~conservative

...
object (self)
  ...
  initializer
    self#log 3 ("Using soundtouch " ^ Soundtouch.get_version_string st ^ ".");
...
let () =
  Lang.add_operator "soundtouch"
    [
      "rate", Lang.float_t, Some (Lang.float 1.0), None;
      "tempo", Lang.float_t, Some (Lang.float 1.0), None;
      "pitch", Lang.float_t, Some (Lang.float 1.0), None;
    ...
    (fun p ->
       ...
       let rate = Lang.to_float (f "rate") in
       let tempo = Lang.to_float (f "tempo") in
       let pitch = Lang.to_float (f "pitch") in

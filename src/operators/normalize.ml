      ...
      let gmax = gmax () in
        for i = offset to AFrame.position buf - 1 do
          for c = 0 to 1 do
            ...
            b.(c).(i) <- b.(c).(i) *. ((float rmsc) *. vold.(c) +. (float (rmsi - rmsc)) *. v.(c)) /. (float rmsi)
          done;
          rmsc <- rmsc + 1;
          if rmsc >= rmsi then
            (
...
let () =
  Lang.add_operator "normalize"
    [
      "target", Lang.float_getter_t 1, Some (Lang.float (-13.)), Some "Desired RMS (dB).";
      "window", Lang.float_t, Some (Lang.float 0.1), Some "Duration of the window used to compute the current RMS power (second).";
      "k_up", Lang.float_getter_t 2, Some (Lang.float 0.005), Some "Coefficient when the power must go up (0 < k_up <1, slowest to fastest).";
      "k_down", Lang.float_getter_t 3, Some (Lang.float 0.1), Some "Coefficient when the power must go down (0 < k_down < 1, slowest to fastest).";
      "threshold", Lang.float_getter_t 4, Some (Lang.float (-40.)), Some "Minimal RMS for activaing gain control (dB).";
      "gain_min", Lang.float_getter_t 5, Some (Lang.float (-6.)), Some "Minimal gain value (dB).";
      "gain_max", Lang.float_getter_t 6, Some (Lang.float 6.), Some "Maximal gain value (dB).";

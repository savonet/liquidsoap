      ...
      List.map
        (fun p ->
           let t = port_t d p in
             ...
             (match Descriptor.port_get_default d ~samplerate:(Fmt.samples_per_second ()) p with
                | Some f ->
                    Some
                      (match t with
                         | Float -> Lang.float f
             ...
             let bounds =
               let min = Descriptor.port_get_min d ~samplerate:(Fmt.samples_per_second ()) p in
               let max = Descriptor.port_get_max d ~samplerate:(Fmt.samples_per_second ()) p in
                         ...
                         | Some f ->
                             (
                               match t with
                                 ...
                                 | Int -> bounds := Printf.sprintf "%s%d <= " !bounds (int_of_float (ceil f))
                                 | Bool -> ()
                             )
                         | None -> ()
                     );
                     bounds := !bounds ^ (norm_string (Descriptor.port_name d p));
                     (
                       match max with
                         | Some f ->
                             (
                               match t with
                                 | Float -> bounds := Printf.sprintf "%s <= %.6g" !bounds f
                                 | Int -> bounds := Printf.sprintf "%s <= %d" !bounds (int_of_float f)
                                 | Bool -> ()
                             )
                         | None -> ()
                     );
...
let get_stereo_io d =
  ...
  let ports = Descriptor.port_count d in
    ...
    match !(fst i), !(snd i), !(fst o), !(snd o) with
      | Some i_l, Some i_r, Some o_l, Some o_r -> (Some [|i_l; i_r|]), [|o_l; o_r|]

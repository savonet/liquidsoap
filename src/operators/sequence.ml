...
object (self)
  ...
  method remaining = (List.hd sources)#remaining
  method abort_track = (List.hd sources)#abort_track
  ...
  method get_frame buf =
    if head_ready then begin
      let hd = List.hd sources in
        hd#get buf ;
        if List.length sources > 1 && Mixer.Buffer.is_partial buf then begin

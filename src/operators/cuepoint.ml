...
object (self)
  ...
  method private cue_in ~buf ~breaks ~delta_pos ~out_pos ~pos seek_time =
    ...
    if seeked_pos = seek_pos then out_pos else
      ...
      match out_pos with
        ...
        | _ ->
           ...
           self#log#info "Seeked %i ticks instead of %i."
             seeked_pos seek_pos ;
           out_pos
        ...
        | None ->
            ...
            let elapsed, out_pos = 
              match in_pos with
                ...
                | Some seek_time ->
                    self#cue_in ~buf ~breaks ~delta_pos ~out_pos ~pos seek_time
            in
              match out_pos with
                | None -> None
                | Some pos -> Some (elapsed,pos)
    in
